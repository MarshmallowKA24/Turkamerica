...
function initializeInterface(deckName, cardsData, unlearnedCount) {
    ...
    if (!viewOnly) {
        attachMarkListeners();
        attachDeleteListeners();  // <-- Agregado aquí para botones eliminar
    } else {
...
function renderCards(cardsData) {
        ...
        cardDiv.innerHTML = `
            <div class="flashcard-inner">
                <div class="flashcard-front">
                    ...
                    ${!viewOnly ? `<button class="btn btn-delete" data-card-id="${card.id}">🗑️ Eliminar</button>` : ''}
                </div>
    ...
    // Adjuntar listeners de eliminación si no es modo solo lectura
    if (!viewOnly) attachDeleteListeners();
}
...
// Adjuntar listeners para los botones de eliminar tarjeta
function attachDeleteListeners() {
    document.querySelectorAll('.btn-delete').forEach(button => {
        button.addEventListener('click', (e) => {
            e.stopPropagation(); // Evita voltear la tarjeta al hacer clic en el botón
            const cardId = button.dataset.cardId;

            if (confirm('¿Estás seguro de que deseas eliminar esta tarjeta?')) {
                sendDeleteCardMessage(cardId);
            }
        });
    });
}

// Enviar mensaje para eliminar tarjeta al padre
function sendDeleteCardMessage(cardId) {
    if (!window.opener || window.opener.closed) {
        console.error('[Study.js] Ventana padre no disponible para eliminar');
    ...
    console.log('[Study.js] Enviando solicitud de eliminación:', message);
    window.opener.postMessage(message, '*');

    // Eliminar la tarjeta de la vista local
